...
| **R6 — Package manager (Chocolatey + MSI + Exe)** *(🟡 wire surface, safety helpers, and concrete drivers shipped; real HTTPS downloader still pending)* | `cmremote-wire`: `PackageProvider` / `PackageInstallAction` / `PackageInstallRequest` / `PackageInstallResult` PascalCase wire types — defaults to `Unknown` so a malformed payload fails closed. `cmremote-platform::packages`: `PackageProviderHandler` async trait + safety helpers — `is_safe_chocolatey_package_id`, `is_safe_chocolatey_version`, `is_safe_msi_file_name`, `is_msi_magic_bytes` (OLE2 `D0 CF 11 E0 A1 B1 1A E1`), `compute_sha256_hex`, `ct_eq_hex` constant-time compare, `is_chocolatey_success_exit_code` matching `Shared.PackageManager.ChocolateyOutputParser.SuccessfulExitCodes`. **Concrete drivers** now ship behind injected `ProcessRunner` + `ArtifactDownloader` traits so the orchestration is unit-tested on Linux CI: `ChocolateyPackageProvider` (allow-listed argv, exit-code mapping including `3010` reboot-required, 30-min timeout, output cap), `UploadedMsiPackageProvider` (download → OLE2 magic + SHA-256 re-verify → `msiexec /i` argv → 60-min run → log-tail capture on failure → unconditional cache cleanup), `ExecutablePackageProvider` (download → SHA-256 verify → exec with operator-vetted silent-install args, install-only, refuses uninstall). The runtime now registers all three via `CompositePackageProvider::register_default_handlers(...)`; the default `RejectingDownloader` (no-op HTTPS client) routes MSI / Executable jobs to a clean operator-facing "this agent is not configured to download package artifacts" failure until the rustls-only HTTPS client lands as a one-line constructor swap. Agent-side `InstallAgentUpdate(downloadUrl, version, sha256)` (slice M3) is now a working handler that re-uses the same `ArtifactDownloader` to fetch + SHA-256 verify the staged artifact and hands off to a pluggable `AgentUpdateInstaller` (default `StubAgentUpdateInstaller` returns a structured "no installer configured" failure so the manifest dispatcher's audit trail stays honest). | Workspace builds clean on stable Rust; `cargo fmt --check` / `cargo clippy --workspace --all-targets -- -D warnings` / `cargo test --workspace --all-targets` all green; `cargo deny check` clean for the new `tempfile` dev-dep + the workspace `uuid` / `async-trait` deps. |
