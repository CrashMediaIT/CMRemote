...
1. **R8 — signed builds + publisher manifest cut-over** — use the now-complete
   R7 desktop transport and the M3 manifest dispatcher seam to ship signed Rust
   agent artifacts, verify publisher metadata, and replace the remaining
   no-op/stub installer path.
2. **End-to-end desktop validation** — add the browser → .NET hub → Rust agent
   → WebRTC video integration job as a CI / lab test, including assertions that
   unattended sessions show host-local connected / disconnected notifications
   and never require local approval prompts.
