...
### Current focus *(end of slices **R2** · **R2a** · **R3** · **R4** · **R5** · **R6** · **R7** *(initial)* / S4 / **M1 milestone complete (M1.1 + M1.2 + M1.3 + M1.4 + M1.5)** + M2 complete + **M3 service + orchestrator landed** + **M4 admin dashboard shipped** — Apr 2026)*
...
[`DesktopTransportProvider`](agent-rs/crates/cmremote-platform/src/desktop.rs)
...
`not_implemented` fall-through — concrete WebRTC capture / encode
driver lands in a follow-up slice without any further dispatch
or wire churn),
the first
...
| **R7 — Desktop transport** *(🟡 wire surface, async trait, and `NotSupportedDesktopTransport` safety stub shipped; concrete WebRTC capture/encode driver pending)* | `cmremote-wire::desktop`: `RemoteControlSessionRequest` / `RestartScreenCasterRequest` / `ChangeWindowsSessionRequest` / `InvokeCtrlAltDelRequest` / `DesktopTransportResult` — PascalCase DTOs mirroring the .NET `IAgentHubClient` shapes byte-for-byte so the same server can drive either agent fleet, defaults that fail closed, JSON + MessagePack round-trip tests, sensitive `access_key` documented as never-logged. `cmremote-platform::desktop`: [`DesktopTransportProvider`](agent-rs/crates/cmremote-platform/src/desktop.rs) async trait (one method per hub call, all returning the same generic `DesktopTransportResult`) + `NotSupportedDesktopTransport` default that returns a structured failure naming the host OS — never panics, never echoes the access key. The agent's dispatcher now routes `RemoteControl`, `RestartScreenCaster`, `ChangeWindowsSession`, and `InvokeCtrlAltDel` invocations through the provider instead of falling through to the generic `not_implemented` completion; malformed `arguments` payloads become structured failures rather than wire-level errors so the audit trail records the bad payload. Concrete WebRTC drivers (capture → encode → ICE → SCTP) plug in alongside the stub without any further dispatch or wire churn. Last and largest slice; tracks Module 5's protocol doc. | Initial slice: workspace tests green (254). Final acceptance bar (unchanged): latency / FPS within 10 % of the .NET/Desktop client on a reference workload. |
