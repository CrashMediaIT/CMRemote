...
### Current focus *(end of slices **R2** · **R2a** · **R3** · **R4** · **R5** · **R6** · **R7** *(initial + R7.b security-contract enforcement + R7.c capture/encode trait surface + R7.d method-surface vectors + R7.f crypto-provider ADR + R7.g signalling DTOs / dispatch + R7.h input-injection / consent-prompt trait surface + R7.i ICE-server-configuration DTOs / guards / vectors + R7.j ProvideIceServers hub-method dispatch + R7.k per-session state machine + WebRtcDesktopTransport skeleton + R7.l webrtc-rs crate-graph supply-chain audit + R7.m webrtc umbrella crate wired behind `webrtc-driver` feature + RTCPeerConnection per-session + R7.n .NET-side WebRTC signalling + ICE-config DTOs and `IAgentHubClient` extensions + R7.n.5 per-session capture pump + R7.n.6 Windows Media Foundation H.264 encoder + encoder→RTP-track wiring + R7 signalling egress HubBoundSignallingEgress + .NET AgentHub forwarding endpoints + per-OS desktop providers Linux + macOS + runtime provider selection by target_os)* / S4 / **M1 milestone complete (M1.1 + M1.2 + M1.3 + M1.4 + M1.5)** + M2 complete + **M3 service + orchestrator landed** + **M4 admin dashboard shipped** — May 2026)*
...
1. **R7 completion** — on-host consent UI (per-OS native dialog wired to
   the `ConsentPrompter` trait) and an end-to-end viewer integration test
   (browser → .NET hub → Rust agent → WebRTC video back to viewer). Once
   these land R7 flips to ✅ and the Rust agent reaches full Windows /
   Linux / macOS desktop parity.
2. **M3 real dispatcher** ✅ seam ready — `ManifestBackedAgentUpgradeDispatcher`
   is wired and R8's publisher-manifest resolver + signed-build pipeline is
   live; the M3 no-op dispatcher can now be replaced with the real one.
3. **M5 — Tests & docs** — `LegacyToV2ConverterTests` golden-vector fixtures,
   `Setup-Wizard.md` operator guide, `Migration.md` admin guide.
4. **Track S remaining** — S5 (CycloneDX SBOM + Sigstore cosign + SLSA v1.0
   ...
   slice).
5. **Live-Postgres integration coverage** (unchanged).
6. **Clean-room server modules 3–6** — `Server.Services` data / auth /
...
| **R7 — Desktop transport** *(🟡 wire surface, async trait, `NotSupportedDesktopTransport` safety stub, security-contract enforcement (R7.b), capture/encode trait surface (R7.c), method-surface vectors (R7.d), WebRTC crypto-provider ADR (R7.f), signalling DTOs / dispatch (R7.g), input-injection / consent-prompt trait surface (R7.h), ICE / TURN server-configuration DTOs / guards / vectors (R7.i), `ProvideIceServers` hub-method dispatch (R7.j), per-session state machine + `WebRtcDesktopTransport` skeleton (R7.k), full `webrtc-rs` crate-graph supply-chain audit (R7.l), and **.NET-side WebRTC signalling + ICE-config DTOs (R7.n)** shipped; **R7.m** (`webrtc` umbrella crate wired behind `webrtc-driver` feature flag; `RTCPeerConnection` per-session construction against `webrtc-cmremote@v0.17.0-cmremote.1`) shipped; **R7.n.5** (per-session capture pump — `CaptureSink` trait + `CapturePump` Tokio actor + `LateBoundCaptureSink` seam + BGRA→NV12 BT.601 converter) shipped; **R7.n.6** (Windows Media Foundation H.264 encoder via `CLSID_MSH264EncoderMFT` + `WindowsVideoEncoderFactory` per-session factory + `WebRtcVideoTrackSink` + encoder→RTP-track chain wired into `build_peer_connection`) shipped; **signalling egress** (`HubBoundSignallingEgress` for agent-originated `SendSdpAnswer` / `SendIceCandidate` invocations + .NET `AgentHub` validation + forwarding endpoints) shipped; **Linux desktop providers** (`cmremote-platform-linux`: X11 capture via `xwd`, H.264 via `ffmpeg`, input via `xdotool`, clipboard via `wl-clipboard` / `xclip`; fails closed when host tools missing) shipped; **macOS desktop providers** (`cmremote-platform-macos`: capture via `screencapture`, H.264 via `ffmpeg`, keyboard via `osascript`, mouse via `cliclick`, clipboard via `pbcopy` / `pbpaste`; fails closed when host tools missing) shipped; **runtime provider selection** (agent selects Windows / Linux / macOS provider bundle by `target_os`, falls back to `NotSupported` stub on error) shipped; on-host consent UI (per-OS native dialog) and E2E viewer integration test pending — see "remaining work" below)* | `cmremote-wire::desktop`: `RemoteControlSessionRequest` / `RestartScreenCasterRequest` / `ChangeWindowsSessionRequest` / `InvokeCtrlAltDelRequest` / `DesktopTransportResult` — PascalCase DTOs mirroring the .NET `IAgentHubClient` shapes byte-for-byte so the same server can drive either agent fleet, defaults that fail closed, JSON + MessagePack round-trip tests, sensitive `access_key` documented as never-logged. `cmremote-platform::desktop`: [`DesktopTransportProvider`](agent-rs/crates/cmremote-platform/src/desktop/mod.rs) async trait (one method per hub call, all returning the same generic `DesktopTransportResult`) + `NotSupportedDesktopTransport` default that returns a structured failure naming the host OS — never panics, never echoes the access key. The agent's dispatcher routes `RemoteControl`, `RestartScreenCaster`, `ChangeWindowsSession`, and `InvokeCtrlAltDel` invocations through the provider instead of falling through to the generic `not_implemented` completion; malformed `arguments` payloads become structured failures rather than wire-level errors so the audit trail records the bad payload. **Slice R7.b** lifts the security contract out of the trait doc and into [`cmremote-platform::desktop::guards`](agent-rs/crates/cmremote-platform/src/desktop/guards.rs): `check_remote_control` / `check_restart_screen_caster` / `check_change_windows_session` enforce cross-org refusal (`request.org_id` must match the agent's own `ConnectionInfo::organization_id`), operator-string sanitisation (256-byte cap, refuse non-printable code points / embedded NUL / DEL, refuse the Unicode bidi-override range that drives "Trojan Source" attacks), and canonical lowercase-UUID `session_id` checks (the only shape the .NET hub ever emits). The `NotSupportedDesktopTransport` stub now runs the guards *before* its OS-not-supported branch, so a hostile request never reaches the per-OS error path; concrete WebRTC drivers will reuse the same helpers. The runtime constructs the stub with the agent's own org id from `ConnectionInfo`. **Slice R7.c** adds [`cmremote-platform::desktop::media`](agent-rs/crates/cmremote-platform/src/desktop/media.rs): `DesktopCapturer` + `VideoEncoder` async traits, `CapturedFrame` (BGRA, stride, μs timestamp) + `EncodedVideoChunk` (bytes, μs, keyframe flag) DTOs, `DesktopMediaError` (`NotSupported(HostOs)` / `Io` / `InvalidParameters`), and `NotSupportedDesktopCapturer` / `NotSupportedVideoEncoder` defaults that return structured errors. Same "trait first, real driver later" pattern slice R6 used for packages — keeps the seam reviewable without pulling in any banned dependencies. **Slice R7.d** ships frozen byte-stable wire-protocol vectors under [`docs/wire-protocol-vectors/method-surface/{remote-control,restart-screen-caster,change-windows-session,invoke-ctrl-alt-del}/`](docs/wire-protocol-vectors/method-surface/) — request + result vectors per method, all round-tripped through both JSON and MessagePack by [`cmremote-wire/tests/vectors.rs`](agent-rs/crates/cmremote-wire/tests/vectors.rs). The wire is now pinned: a future WebRTC driver swap cannot regress the contract. **Slice R7.f** ships [`docs/decisions/0001-webrtc-crypto-provider.md`](docs/decisions/0001-webrtc-crypto-provider.md) — a maintainer-facing ADR laying out the trade-offs between (a) admitting `ring` for the upstream [`webrtc`](https://crates.io/crates/webrtc) crate, (b) forking `webrtc` onto `aws-lc-rs`, and (c) writing a thinner SRTP / DTLS / SCTP stack on top of `aws-lc-rs` directly, plus the security-review questions maintainers must answer before any of those options is actionable. The `deny.toml` ban on `ring` stays in place; the policy bit is unflipped. **Slice R7.g** ships the WebRTC signalling contract: `SdpOffer` / `SdpAnswer` / `IceCandidate` PascalCase DTOs in [`cmremote-wire::desktop::signalling`](agent-rs/crates/cmremote-wire/src/desktop/signalling.rs) with explicit `Kind` discrimination, 16 KiB SDP-body cap (`MAX_SDP_BYTES`) and 1 KiB per-line cap (`MAX_SIGNALLING_STRING_LEN`); matching `check_sdp_offer` / `check_sdp_answer` / `check_ice_candidate` guard helpers that reuse the slice R7.b envelope checks; `on_sdp_offer` / `on_sdp_answer` / `on_ice_candidate` hooks on `DesktopTransportProvider` (default-impl returns "not implemented by this provider", `NotSupportedDesktopTransport` overrides to run guards then return the OS-not-supported failure); dispatch routing for the three new hub methods (`SendSdpOffer` / `SendSdpAnswer` / `SendIceCandidate`); and frozen byte-stable vectors under [`docs/wire-protocol-vectors/method-surface/signalling/`](docs/wire-protocol-vectors/method-surface/signalling/) (offer / answer / candidate / RFC-8838 end-of-candidates marker / success + failure result), all round-tripped through both JSON and MessagePack. Lets the .NET side land its half of the WebRTC negotiation surface against a stable contract while the agent-side WebRTC backend is still gated on the R7.f ADR. **Slice R7.h** adds the input-injection and on-host consent trait surface — same "trait first, real driver later" shape R7.c used for capture/encode. [`cmremote-platform::desktop::input`](agent-rs/crates/cmremote-platform/src/desktop/input.rs) ships `MouseInput` (move/down/up/scroll), `KeyboardInput` (down/up/type-text), and `Clipboard` (read/write text) async traits with `MouseButton` (numbered to match Win32 `XBUTTON*`), `ScrollAxis`, `KeyCode::Char` / `KeyCode::Named(NamedKey)` (every common modifier + arrows + `F(u8)`), and a `DesktopInputError` (`NotSupported(HostOs)` / `ConsentDenied` / `Io` / `InvalidParameters`) whose `Io` variant contractually MUST NOT carry typed-text or clipboard payload bytes. Three `NotSupported*` defaults fail closed naming the host OS. [`cmremote-platform::desktop::consent`](agent-rs/crates/cmremote-platform/src/desktop/consent.rs) ships a `ConsentPrompter` async trait, a `ConsentRequest` DTO whose [`sanitised`] constructor reuses [`super::guards::validate_operator_string`] so a hostile org name with bidi-overrides / NUL / DEL / over-length payload never reaches the on-host prompt, a `ConsentDecision` (`Granted` / `Denied { reason }` / `Timeout`), a `DEFAULT_CONSENT_TIMEOUT = 30 s` mirroring the .NET implementation, and a fail-closed [`DenyAllConsentPrompter`] default the runtime registers when no per-OS UI is wired in (a session reaches a host with no consent UI configured and is *refused*, not silently allowed). All R7.h types are exported through the `desktop` module's prelude. No new dependencies; no banned crates; no wire churn. **Slice R7.i** adds the ICE / TURN server-configuration contract the eventual WebRTC peer connection needs before it can negotiate through restricted networks. [`cmremote-wire::desktop::signalling`](agent-rs/crates/cmremote-wire/src/desktop/signalling.rs) gains PascalCase `IceServer` (`urls` + optional `username` + sensitive `credential` + `credential_type`), `IceServerConfig` (`ice_servers` + `ice_transport_policy`), `IceCredentialType` (`Password` default; `Oauth` reserved on the wire but failed closed by the guard until the OAuth pipeline lands), and `IceTransportPolicy` (`All` / `Relay`) DTOs, plus per-config caps (`MAX_ICE_SERVERS = 8`, `MAX_URLS_PER_ICE_SERVER = 4`, `MAX_ICE_URL_LEN = 512`, `MAX_ICE_CREDENTIAL_LEN = 512`). [`check_ice_server_config`](agent-rs/crates/cmremote-platform/src/desktop/guards.rs) enforces a `stun:` / `stuns:` / `turn:` / `turns:` URL-scheme allow-list (refusing `http(s)://`, `file://`, scheme-relative `//host`, bare `host:port`, and `JAVASCRIPT:`-style probes — without ever echoing the hostile URL into the rejection message), refuses embedded whitespace / controls / NUL / DEL / bidi-overrides in any URL, requires `username` + `credential` for `turn(s):` URLs and forbids them for plain `stun(s):` (per W3C `RTCIceServer`), length-caps the **sensitive** `credential` and refuses hostile bytes (control / NUL / DEL / bidi-override) without ever including the credential's printable bytes in the rejection message, and fails closed on `IceCredentialType::Oauth`. Frozen byte-stable vectors under [`docs/wire-protocol-vectors/method-surface/ice-config/`](docs/wire-protocol-vectors/method-surface/ice-config/) — typical stun + turn(s) config and a relay-only config — round-trip through JSON + MessagePack. The .NET hub will eventually deliver an `IceServerConfig` either as a field on the existing `RemoteControlSessionRequest` (under the same wire-protocol bump rules) or via a dedicated `ProvideIceServers` hub method; that integration step was deliberately deferred so the DTO contract could land first and the .NET side could build against frozen vectors. **Slice R7.j** retires that deferral by wiring the dedicated `ProvideIceServers(iceServerConfig, sessionId, accessKey, requesterName, orgName, orgId)` hub method through dispatch end-to-end. [`cmremote-wire::desktop::signalling`](agent-rs/crates/cmremote-wire/src/desktop/signalling.rs) gains a `ProvideIceServersRequest` PascalCase DTO whose envelope mirrors `SdpOffer` verbatim plus a nested `IceServerConfig`, so the agent's existing operator-identity guards apply uniformly to the new method. [`check_provide_ice_servers`](agent-rs/crates/cmremote-platform/src/desktop/guards.rs) chains the slice R7.b envelope checks (cross-org, canonical-UUID `session_id`, sanitised operator strings) with the slice R7.i per-server checks (URL allow-list / cap enforcement / TURN credential pairing / sensitive-credential redaction / `Oauth` fail-closed) and refuses every hostile shape *without ever reading the sensitive `access_key`* — pinned by a property-style test that drives every refusal branch with a fixed `secret-access-key` payload and asserts the rejection message never echoes it. The `DesktopTransportProvider` async trait grows an `on_provide_ice_servers` hook (default-impl returns "not implemented by this provider", `NotSupportedDesktopTransport` overrides to run the guard then return the structured "not supported on `<host_os>`" failure). The agent dispatcher learns the new `MethodName::ProvideIceServers` allow-list entry and routes to a matching `handle_provide_ice_servers` handler that decodes the single-arg `arguments` payload (malformed shapes become structured failures in the audit trail rather than wire-level errors). Frozen byte-stable vectors land under [`docs/wire-protocol-vectors/method-surface/provide-ice-servers/`](docs/wire-protocol-vectors/method-surface/provide-ice-servers/) — request + result-failure — both round-tripped through JSON + MessagePack. No new dependencies; no banned crates; no driver code; no churn to the existing four method-surface methods or the three R7.g signalling methods. **Slice R7.n** (.NET-side WebRTC signalling + ICE-config DTOs and `IAgentHubClient` extensions): retires the long-standing R7 deferral that the .NET hub couldn't typed-dispatch the R7.g / R7.i / R7.j hub methods to a Rust-agent connection. Adds [`Shared/Enums/{SdpKind,IceCredentialType,IceTransportPolicy}.cs`](Shared/Enums/) — all carrying `[JsonConverter(typeof(JsonStringEnumConverter))]` so `"Offer"` / `"Password"` / `"All"` / `"Relay"` round-trip as strings, matching the Rust `#[serde(rename_all = "PascalCase")]` enums byte-for-byte. Adds [`Shared/Dtos/{SdpOfferDto,SdpAnswerDto,IceCandidateDto,IceServerDto,IceServerConfigDto,ProvideIceServersRequestDto,DesktopTransportResultDto}.cs`](Shared/Dtos/) — PascalCase property names matching the frozen Rust JSON byte-for-byte, including the `[JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]` on `DesktopTransportResultDto.ErrorMessage` that mirrors the Rust `#[serde(skip_serializing_if = "Option::is_none")]` on `error_message` so the success vector serialises without the field. Mirrors the documented Rust caps as `public const` on the relevant DTOs (`MaxSdpBytes` = 16 KiB, `MaxSignallingStringLen` = 1 KiB, `MaxIceServers` = 8, `MaxUrlsPerIceServer` = 4, `MaxIceUrlLen` = 512, `MaxIceCredentialLen` = 512). [`Shared/Interfaces/IAgentHubClient.cs`](Shared/Interfaces/IAgentHubClient.cs) gains `SendSdpOffer` / `SendSdpAnswer` / `SendIceCandidate` / `ProvideIceServers` so `IHubContext<AgentHub, IAgentHubClient>` can typed-dispatch the new methods to a Rust-agent connection without going through the legacy `SendDtoToClient` byte channel. The legacy .NET `AgentHubConnection` implementation in [`Agent/Services/AgentHubConnection.cs`](Agent/Services/AgentHubConnection.cs) ships fail-soft `Debug`-level no-op stubs for the four new methods so the legacy build keeps green during the cut-over (legacy viewers paired with a legacy agent continue to use the byte-array `DtoWrapper` channel for SDP / ICE traffic; the typed methods are honoured by the Rust agent only). Tests in [`Tests/Shared.Tests/WebRtcSignallingDtoTests.cs`](Tests/Shared.Tests/WebRtcSignallingDtoTests.cs) (+17 cases) round-trip every frozen R7.g / R7.i / R7.j vector under [`docs/wire-protocol-vectors/method-surface/{signalling,ice-config,provide-ice-servers}/`](docs/wire-protocol-vectors/method-surface/) through `System.Text.Json` and assert (a) the deserialised DTO matches the documented values, (b) the re-serialised JSON has *exactly* the same property set as the original (no missing fields, no extra fields, no renamed keys, no enum form drift, including the RFC 8838 end-of-candidates marker's explicit-`null` `SdpMid` / `SdpMlineIndex`), (c) the `public const` caps match the documented Rust values, (d) defaults fail closed (`DesktopTransportResultDto.Success` defaults to `false`; `IceServerDto.CredentialType` defaults to `Password`; `IceServerConfigDto.IceTransportPolicy` defaults to `All`), and (e) the enums serialise as PascalCase strings rather than the System.Text.Json default of integers. No new dependencies; no banned packages; no SignalR hub-protocol churn. Closes the "long-standing assumption that the .NET hub couldn't talk to a Rust agent without going through the legacy byte channel" — the .NET server can now `await _agentHub.Clients.Client(connId).ProvideIceServers(request)` and the Rust agent's `handle_provide_ice_servers` receives the byte-identical PascalCase payload its `check_provide_ice_servers` guard already validates. **Remaining work for the final R7 acceptance bar** (each item now has a clearer next step thanks to R7.f / R7.g / R7.h / R7.i / R7.j / R7.k / R7.l / R7.m): (1) per-OS capture (DXGI Desktop Duplication / ScreenCaptureKit / PipeWire) and encode (Media Foundation / VideoToolbox / VAAPI / libx264) implementations of the R7.c traits, plumbed through `WebRtcDesktopTransport`'s capture pump → encoder sink → `RTCRtpSender` track wiring (the R7.m peer connection is connected today but emits no media); (2) per-OS implementations of the R7.h input traits (`SendInput` on Windows, `uinput` / `XTest` on Linux, Quartz Event Services on macOS) and a real GUI consent prompter for each host (Win32 dialog / GTK / Cocoa) replacing `DenyAllConsentPrompter`; (3) the .NET hub-side glue that emits / consumes the R7.g signalling DTOs and the R7.i `IceServerConfig` through the typed `IAgentHubClient` methods landed in R7.n (`SendDtoToClient` / `SendDtoToViewer` is the legacy fallback); (4) wiring the agent runtime's `LoggingSignallingEgress` default to a real hub-bound implementation that invokes `IAgentHub.SendSdpAnswer` / `SendIceCandidate` server-bound methods; (5) a reference benchmark harness running both the Rust agent and the .NET Desktop client on identical hardware to demonstrate the latency/FPS bar. **Slice R7.m** (concrete WebRTC peer-connection construction): replaces every `DRIVER_PENDING_MESSAGE` stub return on the `WebRtcDesktopTransport` provider methods with real `webrtc-rs` operations against the `[patch.crates-io]` pin to `CrashMediaIT/webrtc-cmremote@v0.17.0-cmremote.1` that the R7.l audit + Spike Steps 10–18 unlocked. `on_provide_ice_servers` translates the slice R7.i `IceServerConfig` into a [`webrtc::peer_connection::configuration::RTCConfiguration`](agent-rs/crates/cmremote-platform/src/desktop/webrtc_pc.rs) (defence-in-depth: never forwards a non-`Password` credential into the upstream stack even though the slice R7.j guard already refuses every other variant) and constructs an `RTCPeerConnection` via a shared `PeerConnectionFactory` (one `MediaEngine` registered with the upstream default codec set, shared across every per-session connection — same pattern the upstream `examples/` directory uses). `on_sdp_offer` ensures a peer connection exists (lazy-creating one with the default `RTCConfiguration` if no `ProvideIceServers` has been seen — the .NET hub may skip it when the viewer reuses defaults), `set_remote_description`s the offer, `create_answer`s, `set_local_description`s the answer, and pushes the answer through a new [`SignallingEgress`](agent-rs/crates/cmremote-platform/src/desktop/signalling_egress.rs) async-trait seam that the agent runtime (a future slice) wires to `IAgentHub.SendSdpAnswer`; the default `LoggingSignallingEgress` `tracing::warn!`s every drop so the audit log captures untransmitted answers / candidates. `on_sdp_answer` `set_remote_description`s onto the existing PC; `on_ice_candidate` `add_ice_candidate`s after translating the wire shape to [`webrtc::ice_transport::ice_candidate::RTCIceCandidateInit`](agent-rs/crates/cmremote-platform/src/desktop/webrtc_pc.rs); `change_windows_session` and the idle sweep close the PC alongside the matching pump and registry record; `remote_control`'s replace-on-duplicate path closes the prior PC alongside the prior pump. Per-PC alive flags gate the `on_peer_connection_state_change` registry-mutation handler so a late-arriving `Closed` event from a torn-down PC cannot stomp the freshly-opened replacement session. Locally-trickled candidates from the upstream ICE agent are forwarded through the `SignallingEgress` only while the alive flag is set, so a closed PC's gathering tail cannot leak candidates addressed to a dead viewer. The capture-pump → encoder → `RTCRtpSender` track wiring is still pending — a session built today negotiates a real DTLS / SCTP transport but emits no media (the `DiscardingCaptureSink` from R7.n.5 still receives every frame). 274 platform-crate `--features webrtc-driver` tests green (was 251 → +23 new tests covering: happy-path PC build through `on_provide_ice_servers`, lazy-build through `on_sdp_offer`, SDP answer captured through `CapturingSignallingEgress`, ICE candidate accepted by the upstream stack, replace-on-duplicate drops the prior PC without stomping the new session, `ChangeWindowsSession` closes registry+pump+PC together, idle sweep closes registry+pump+PC together, every out-of-order signalling path refused with a precise message that never echoes the sensitive `access_key`, every guard refusal happens before any PC mutation including ice-server URL allow-list refusal happening before the wire-translation step, and the `webrtc_pc` translation layer's defence-in-depth refusal of non-`Password` credentials). Workspace test count 453 → 490 with `--features cmremote-agent/webrtc-driver`. No new dependencies (the `webrtc` crate was already an optional dep landed in R7.k); no banned crates (the `[patch.crates-io]` pin still resolves through `aws-lc-rs`, `cargo deny check` unchanged); no wire churn. **Slice R7.k** (per-session state machine + `WebRtcDesktopTransport` skeleton): adds [`cmremote-platform::desktop::session`](agent-rs/crates/cmremote-platform/src/desktop/session.rs) — a `DesktopSessionState` enum (`Initializing` → `IceConfigured` → `NegotiatingSdp` → `Connected` → `Closed` plus the `Connected` → `NegotiatingSdp` renegotiation edge), a `DesktopSession` struct with `last_activity` tracking, a `DesktopSessionRegistry` keyed by canonical lowercase-UUID `session_id` with replace-on-duplicate semantics for `RemoteControl`, an idle-timeout sweep (`DEFAULT_IDLE_TIMEOUT = 120 s`), structured `tracing::info!` audit events on every transition (`session_id` / `viewer_connection_id` / `from_state` / `to_state` / `event` / optional `close_reason` fields), and a debug-only canonical-UUID assertion as defence in depth (the wire-protocol guards remain the first line of validation). Adds [`cmremote-platform::desktop::webrtc::WebRtcDesktopTransport`](agent-rs/crates/cmremote-platform/src/desktop/webrtc.rs) — a concrete `DesktopTransportProvider` gated behind a default-off `webrtc-driver` cargo feature on `cmremote-platform` (and forwarded through `cmremote-agent`'s `webrtc-driver` feature). The provider runs the existing R7.b / R7.g / R7.j guards *before* any registry mutation, opens / replaces / closes sessions through the registry, refuses out-of-order signalling (`SendSdpOffer` without a prior `RemoteControl` is a precise rejection — never echoes the `access_key`), and stubs the actual peer-connection construction with a `DesktopTransportResult::failed("WebRTC driver pending — peer-connection construction stub (host_os=…)")` once the lifecycle work is done. The runtime swaps in `WebRtcDesktopTransport` when `--features cmremote-agent/webrtc-driver` is set; the default build keeps `NotSupportedDesktopTransport` so the R7.l audit can land without affecting any production build. Concrete WebRTC drivers (capture → encode → ICE → SCTP) plug in alongside the stub without any further dispatch or wire churn. Tracks Module 5's protocol doc. **Slice R7.l** (full `webrtc-rs` crate-graph supply-chain audit): closes the long-standing assumption that the spike's `webrtc-dtls`-only audit covered the rest of the upstream `webrtc-rs/webrtc` workspace. [`docs/decisions/0001-webrtc-crate-graph-audit.md`](docs/decisions/0001-webrtc-crate-graph-audit.md) records the per-sub-crate verdict against `webrtc-rs/webrtc@v0.17.0` (the final feature release of the Tokio-coupled API): four **`needs-fork`** verdicts (`webrtc` umbrella, `webrtc-dtls` rebase from v0.5.4 to v0.17, `webrtc-stun`, `webrtc-turn` — every one a mechanical `ring::* → aws_lc_rs::*` symbol substitution of the shape the spike already proved out), one **`clean *(policy gate)*`** verdict (`webrtc-srtp` is RustCrypto-only by default but exposes an opt-in `openssl` feature that would trip the existing `openssl-sys` ban — R7.m must consume it with default features only and add a `cargo tree -i openssl-sys` smoke check), and ten **`clean`** verdicts (`webrtc-ice`, `webrtc-mdns`, `webrtc-interceptor`, `webrtc-data`, `webrtc-media`, `webrtc-util`, `webrtc-rtcp`, `webrtc-rtp`, `webrtc-sctp`, `webrtc-sdp` — they ride on the four forks transitively but need no source changes). The audit recommends **Option L2 — one monorepo fork mirroring upstream's workspace layout** (repurpose `CrashMediaIT/webrtc-cmremote` from a dtls-only fork to a full `webrtc-rs/webrtc@v0.17.0` fork on a fresh `cmremote/v0.17.0-aws-lc-rs` branch tagged `v0.17.0-cmremote.1`), so `agent-rs/Cargo.toml` ends up with four `[patch.crates-io]` entries that all reference the same git tag plus a per-sub-crate `path = "..."`. Matching **Steps 10–18** appended to [`docs/decisions/0001-spike-fork-instructions.md`](docs/decisions/0001-spike-fork-instructions.md) cover the rebase, the per-sub-crate sed patch, the `Cargo.toml` feature-list swaps for `rcgen` (`default-features = false, features = ["aws_lc_rs", "pem"]`) and `rustls` (`features = ["std", "aws_lc_rs"]` instead of `["std", "ring"]`), the five-target cross-compile gate, the new tag, the wiring step that the R7.m PR will execute, and the rebase cadence. [`agent-rs/deny.toml`](agent-rs/deny.toml) gains a documentation block plus commented-out placeholder `[sources].allow-git` entries for the four prospective Option L1 sub-repos (so a maintainer flipping to Option L1 has the URLs at the ready); the Option L2 path needs no further allow-list change because the existing `https://github.com/CrashMediaIT/webrtc-cmremote` host entry already covers the new monorepo tag. The `[bans].deny` `ring` entry stays untouched — its continued presence is still the load-bearing assertion that ADR-0001 Option B is being honoured. No new dependencies, no Cargo.toml change, no source-code change under `agent-rs/`; the production build is byte-identical and `cargo deny check` is unchanged. R7.l is **the gating prerequisite for R7.m** — without the audit and runbook, the R7.m PR has no manifest of which forks to create or which patch entries to add. With R7.l shipped, R7.m's only outstanding human-side dependency is creating the monorepo fork branch + tag in the existing `CrashMediaIT/webrtc-cmremote` repository (out-of-band from this monorepo because the cloud agent cannot push to other repos). | Slices R7 (initial) + R7.b + R7.c + R7.d + R7.f + R7.g + R7.h + R7.i + R7.j + R7.k + R7.l + R7.n: workspace tests green (399; was 375 → +24 R7.j tests split across the wire DTO (+3 PascalCase round-trip / msgpack / fail-closed defaults), the `check_provide_ice_servers` guard helper (+10 covering happy path / cross-org / non-canonical session id / bidi-override in operator strings / control character in viewer connection id / hostile ICE URL / over-cap server count / `Oauth` fail-closed / sensitive `access_key` redaction across every rejection branch / sensitive TURN credential redaction), the `on_provide_ice_servers` provider hook (+5 covering OS-not-supported failure shape / `access_key` non-leak / cross-org guard runs before the OS branch / hostile ICE URL refused before the OS branch / default trait method returns "not implemented"), the `handle_provide_ice_servers` dispatch handler (+4 covering routing / cross-org refusal at handler level / hostile URL refusal / malformed JSON arguments shape), and the corpus vectors (+2 request + result-failure round-tripped through JSON and MessagePack). R7.l is documentation + commented-out `deny.toml` placeholders only; no test count change, no Cargo.toml or source-code change, `cargo deny check` unchanged. R7.n adds .NET-side coverage: `Tests/Shared.Tests` 14 → 31 (+17 vector round-trip / cap-mirror / fail-closed-default / PascalCase-enum-serialisation cases); `Tests/Server.Tests` 233 unchanged (the new DTOs are additive on `IAgentHubClient`); legacy `Agent/Agent.csproj` builds clean against the extended interface via the four `Debug`-logged no-op stubs. The R7.i deferral note ("dispatch wiring deliberately deferred") is now retired; the R7.n .NET-side glue lets the legacy server typed-dispatch every R7.g / R7.i / R7.j hub method straight to a Rust-agent connection. R7.m / R7.n.5 / R7.n.6 / signalling egress / Linux + macOS providers are now shipped (PRs #25–#30, May 2026). **Remaining work**: (1) on-host consent UI — wire a platform-native dialog to the `ConsentPrompter` trait on Windows (TaskDialog), Linux (zenity / kdialog), and macOS (osascript); (2) E2E viewer integration test — full round-trip from a browser viewer through the .NET hub and Rust WebRTC driver back to the viewer with live H.264 video. Final acceptance bar (unchanged): latency / FPS within 10 % of the .NET/Desktop client on a reference workload. |
