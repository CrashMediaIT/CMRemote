...
### Current focus *(end of slices **R2** · **R2a** · **R3** · **R4** · **R5** · **R6** · **R7** *(initial + R7.b security-contract enforcement + R7.c capture/encode trait surface + R7.d method-surface vectors)* / S4 / **M1 milestone complete (M1.1 + M1.2 + M1.3 + M1.4 + M1.5)** + M2 complete + **M3 service + orchestrator landed** + **M4 admin dashboard shipped** — Apr 2026)*
...
DTOs in
[`cmremote-wire::desktop`](agent-rs/crates/cmremote-wire/src/desktop.rs);
...
round-tripped through both JSON and MessagePack — concrete WebRTC
capture / encode driver lands in a follow-up slice without any
further dispatch or wire churn),
the first
