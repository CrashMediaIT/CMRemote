...
`ServerVerificationToken` and bearer tokens are file-only.

`ConnectionInfo.Debug` redaction: implementations must guarantee
that `ServerVerificationToken` is never written to logs or panic
output. The Rust crate enforces this with a hand-written `Debug`
impl pinned by `connection_info_redacts_token` in
`cmremote-wire`.
